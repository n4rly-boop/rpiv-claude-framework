...
3. **Present plan**: List files per commit + messages. Use `AskUserQuestion`: "I plan to create N commit(s). Shall I proceed?" **NEVER commit without actual user approval.**
4. **Execute only after user confirms**: `git add` specific files (never `-A` or `.`), create commits, show `git log --oneline -n N`
